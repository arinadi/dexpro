...
- Re-run on the real device confirmed the group-retry restructure works as intended (`Termux:X11` and `graphics` groups installed cleanly) but surfaced a second bad name: `pulseaudio-utils` — Termux doesn't split PulseAudio into a separate `-utils` package the way Debian does; the single `pulseaudio` package (matching XLabs) provides both the daemon and `pactl`, which `app/native/audio.py` needs. Fixed.
- After a successful `install.py` run, `dexpro` (via the `$PREFIX/bin/dexpro` symlink `install.py::link_launcher()` creates) failed with `ModuleNotFoundError: No module named 'app'`. Root cause: the launcher script resolved `REPO_DIR` from `dirname "$0"` without following the symlink — invoked via PATH, `$0` stays as the symlink's own path (`$PREFIX/bin`), so it `cd`'d there instead of the repo and `app.app` wasn't importable. XLabs' `xlabs` launcher already solves this with `readlink -f "$0"` plus an explicit `PYTHONPATH` (its own comment: "readlink -f resolves $PREFIX/bin/xlabs back to the repo"). Applied the same pattern to `dexpro`.
- With the launcher fixed, `dexpro` failed with `ModuleNotFoundError: No module named 'textual'`. Root cause: `install.py` never had an `install_libs()` step at all — `pyproject.toml` declares `textual>=8.2` as a dependency, but nothing in the install flow ever ran `pip install`. XLabs' `install.py` has this step with a 3-attempt fallback chain (plain, `--break-system-packages`, `--user`) for Termux's PEP 668 externally-managed Python; dexpro's was missing outright, not just wrong. Added, mirroring XLabs. Not yet re-confirmed on-device.
