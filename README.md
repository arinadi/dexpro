...
| Settings | Real, live-editable choices — GPU profile (with a Bench button to re-run the real glmark2 benchmark on demand and save the winner), storage link mode (backed up automatically the first time it's enabled), termux-x11 rendering, audio (off by default, matching dextop; a Test button generates a real tone and plays it, not just checking the daemon exists) — saved immediately on change; a Termux terminal font applier (copies a `.ttf` you already have to `~/.termux/font.ttf`, the real Termux mechanism); plus a working Uninstall |
