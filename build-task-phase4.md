...
Real finding, not hypothetical: **`pactl info` autospawns a PulseAudio daemon as a side effect** of merely checking whether one is running. Caught by a container test run where `check_audio()` unexpectedly reported `ok=True` on a fresh container that had never started PulseAudio — the *check itself* had silently started it. This is the wrong behavior for a Doctor status probe (checking should never provision the thing it's checking for). Fixed by setting `PULSE_AUTOSPAWN=0` in `audio.py::is_running()`'s subprocess environment, making it a genuine read-only probe. Also confirmed: `termux-wake-lock` is present even in a bare `termux-docker` container (it ships in core `termux-tools`, matching the Phase 1 web research finding) — a test that assumed its absence in any non-Termux environment was simply wrong and was relaxed to test the check's contract (returns a definite bool, never crashes) rather than assert a specific environment's package state.

**Confirmed working end-to-end against a real `proot-distro` container** (reusing Phase 2's create/user flow): `checks.check_apt_lists()` and `checks.check_user_uid_mapped()` both correctly reported OK against a genuinely created container with a genuinely host-UID-mapped user. `backup.backup_container()` produced a real ~60MB archive via `proot-distro backup -c zstd`, confirmed to exist on disk.

**Native home backup/restore is fully tested locally** using real `tar` (available via Git Bash/MSYS on this dev machine, no Termux needed for this half) — round-trip content match, move-existing-aside-not-overwrite behavior, and atomic-write (no partial backup ever listed) all verified by actual test runs, not assumed.

**Not yet exercised**: `doctor/electron.py`'s Electron detection/patching against a real Electron app (only pure Exec-line-rewrite logic tested — no real `chrome-sandbox` binary was set up in a test container); `doctor/duplicates.py`'s cross-check against a container that actually has an overlapping tool installed (only the "nothing overlaps" and "proot-distro missing" paths were exercised); the Termux:X11 signing-variant check remains honestly scoped to "package installed" only — full signing-variant verification was never implemented, a pre-existing scope decision from when the check was written, not a gap introduced this phase (see checks.py's own docstring); and the Doctor/Backup TUI screens' Fix(N)/backup-button flows haven't been driven against a real seeded-failure scenario, only navigation-tested via Pilot.
